def productExceptSelf(nums):
    ...
    #compute the prefixes and store in prefix array 
    pre = 1
    for i in range(len(nums)):
        prefix[i] = pre
        pre *= nums[i]

    #compute the postfixes and store in postfix array 
    post = 1
    for i in range(len(nums) - 1, -1, -1):
        postfix[i] = post
        post *= nums[i]

    #iterate over nums array and multiply prefix[index] * postfix[index]
    for index in range(len(nums)):
        output[index] = prefix[index] * postfix[index]
